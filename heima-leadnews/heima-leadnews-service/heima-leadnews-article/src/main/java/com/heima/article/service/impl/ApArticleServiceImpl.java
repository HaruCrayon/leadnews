...
//        }
     ...
     * @param tag
     */
    private void replaceDataToRedis(ApArticle apArticle, Integer score, String tag) {
        String articleListStr = cacheService.get(tag);
        if (StringUtils.isNotBlank(articleListStr)) {
            ...
            //缓存到redis
            hotArticleVoList = hotArticleVoList.stream().sorted(Comparator.comparing(HotArticleVo::getScore).reversed()).collect(Collectors.toList());
            cacheService.set(tag, JSON.toJSONString(hotArticleVoList));
        }
    }
